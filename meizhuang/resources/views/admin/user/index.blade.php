...
<div >
    ...
    <div class="page-container">
        <div class="page-container">
            <div class="text-c">
                <form action="{{url('admin/user/select')}}" method="post">
                    @csrf
                    <input type="text" name="id"  placeholder="    昵称" style="width:250px" class="input-text">
                    <button   class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i> 搜索用户</button>
                </form>
            ...
            <table class="table table-border table-bordered table-bg table-hover table-sort">
                ...
                @foreach($data as $v)
                    <tr class="text-c">
                        ...
                        <td>{{$v->shouhuoren}}</td>
                        <td>{{$v->dizhi}}</td>
                        <td> <a onclick="Dongjie({{$v->id}})">@if($v->dongjie==0)<span class="label label-success radius">否</span>  @else <span class="label label-danger radius">是</span>@endif</a></td>
                        <td class="td-manage"><a title="编辑" href="{{url('admin/user/'.$v->id.'/edit')}}" style="text-decoration:none"><i style="font-size: 20px" class="Hui-iconfont">&#xe6df;</i></a>
...
<script type="text/javascript">
    ...
    function Dongjie(id){
        layer.confirm('您确定冻结此用户吗？', {
            btn: ['确定','取消'] //按钮
        }, function(){
            $.post("{{url('admin/user/dongjie/')}}/"+id ,{'_method':'get','_token':"{{csrf_token()}}"},function (data) {
                if ((data.status==0)) {
                    location.href=location.href;
                    layer.msg(data.msg,{icon:6});
                }else {
