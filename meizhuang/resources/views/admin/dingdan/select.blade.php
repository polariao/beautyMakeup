...
<div >
	...
	<div class="page-container">
		<div class="text-c">
			<form action="{{url('admin/dingdan/select')}}" method="post">
				@csrf
				<input type="text" name="id"  placeholder="    收货人 / 单号" style="width:250px" class="input-text">
			...
			<table class="table table-border table-bordered table-bg table-hover table-sort">
				...
				@foreach($data as $v)
					<tr class="text-c va-m">
						...
						<td>{{$v->dizhi}}</td>
						<td>{{$v->youbian}}</td>
						<td> <a onclick="Fahuo({{$v->id}})">@if($v->status==1)<span class="label label-success radius">是</span>  @else <span class="label label-danger radius">否</span>@endif</a></td>
						<td>{{$v->tel}}</td>
						<td>{{$v->number}}</td>
						<td>{{date('Y-m-d H:i:s',$v->time)}}</td>
						<td>{{$v->danjia}}</td>
						<td>{{$v->danjia*$v->number}}</td>
						<td class="td-manage"> <a style="text-decoration:none" class="ml-5" href="{{url('admin/dingdan/'.$v->id.'/edit')}}" title="编辑"><i style="font-size: 20px" class="Hui-iconfont">&#xe6df;</i></a>
	...
	<script type="text/javascript">
        ...
        function Fahuo(id){
            layer.confirm('您确定进行此操作吗？', {
                btn: ['确定','取消'] //按钮
            }, function(){
                $.post("{{url('admin/dingdan/fahuo/')}}/"+id ,{'_method':'get','_token':"{{csrf_token()}}"},function (data) {
                    if ((data.status==0)) {
                        location.href=location.href;
                        layer.msg(data.msg,{icon:6});
                    }else {
