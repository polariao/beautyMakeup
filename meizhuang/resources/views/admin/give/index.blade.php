...
<div >
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 管理员管理 <span class="c-gray en">&gt;</span> 授权中心 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
            ...
            <table class="table table-border table-bordered table-bg table-hover table-sort">
                ...
                @foreach($data as $v)
                    <tr class="text-c">
                        ...
                        <td>{{$v->name}}</td>
                        <td class="td-manage">
                            <a title="授权" href="javascript:;" onclick="@if(Session::get('admin.name')=='admin')  give({{$v->id}});@else alert('非超级管理员，权限不足');  @endif" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe63f;</i></a></td>
                        <td class="td-manage">
                            <a title="删除" href="javascript:;" onclick="@if(Session::get('admin.name')=='admin')  delCate({{$v->id}});@else alert('非超级管理员，权限不足');  @endif" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
                    </tr>
                @endforeach
                </tbody>
            </table>
...
<script type="text/javascript">
    ...
    function give(user_id){
        layer.confirm('您确定授权吗？', {
            btn: ['确定','取消'] //按钮
        }, function(){
            $.post("{{url('admin/give/')}}/"+user_id ,{'_method':'put','_token':"{{csrf_token()}}"},function (data) {
                if ((data.status==0)) {
                    location.href=location.href;
                    layer.msg(data.msg,{icon:6});
                }else {
