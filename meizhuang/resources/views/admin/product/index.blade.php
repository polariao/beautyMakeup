...
<div >
	...
	<div class="page-container">
		<div class="text-c">
			<form action="{{url('admin/product/select')}}" method="post">
				@csrf
		...
		<div class="mt-20">
			<table class="table table-border table-bordered table-bg table-hover table-sort">
				...
				<tr class="text-c va-m">
                    ...
                    <td>{{$v->spdengji}}</td>
                    <td>{{$v->pinpai}}</td>
                    <td>@if($v->time<time()+8*60*60&&$v->status==1)<span class="label label-success radius">是</span>  @else <span class="label label-danger radius">否</span>@endif</td>
                    <td>{{$v->huiyuanjia}}</td>
                    <td>{{$v->shichangjia}}</td>
                    <td>{{$v->chengben}}</td>
					<td>{{$v->shuliang}}</td>
