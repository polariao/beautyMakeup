...
<div >
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 商品管理 <span class="c-gray en">&gt;</span> 已下架商品列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	<div class="page-container">
		<div class="text-c">
			<form action="{{url('admin/product/select3')}}" method="post">
				@csrf
				<input type="text" name="id"  placeholder="    商品名称" style="width:250px" class="input-text">
				<button   class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i> 搜索商品</button>
			</form>
		...
		<div class="mt-20">
			<table class="table table-border table-bordered table-bg table-hover table-sort">
				...
				<tr class="text-c va-m">
                    ...
                    <td>{{$v->spdengji}}</td>
                    <td>{{$v->pinpai}}</td>
                    <td><span class="label label-danger radius">已下架</span></td>
                    <td>{{$v->huiyuanjia}}</td>
                    <td>{{$v->shichangjia}}</td>
                    <td>{{$v->chengben}}</td>
					<td>{{$v->shuliang}}</td>
					...
					<td >{!! $v->spjianjie !!}</td>
					<td class="td-manage">
						<a title="重新上架" href="javascript:;" onclick="updateStatus({{$v->id}})" class="ml-5" style="text-decoration:none"><i style="font-size: 20px" class="Hui-iconfont">&#xe6dc;</i></a>
...
<script type="text/javascript">
    ...
    function updateStatus(id){
        layer.confirm('您确定重新上架吗？', {
            btn: ['确定','取消'] //按钮
        }, function(){
            $.post("{{url('admin/shan/shang/')}}/"+id ,{'_method':'get','_token':"{{csrf_token()}}"},function (data) {
                if ((data.status==0)) {
                    location.href=location.href;
                    layer.msg(data.msg,{icon:6});
                }else {
