...
<div >
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 商品管理 <span class="c-gray en">&gt;</span> 等待商品列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
	<div class="page-container">
        <div class="page-container">
            <div class="text-c">
                <form action="{{url('admin/product/select1')}}" method="post">
                    @csrf
		...
		<div class="mt-20">
			<table class="table table-border table-bordered table-bg table-hover table-sort">
				...
				<tr class="text-c va-m">
                    ...
                    <td>{{$v->spdengji}}</td>
                    <td>{{$v->pinpai}}</td>
                    <td><span class="label label-success radius">等待上架</span></td>
                    <td>{{$v->huiyuanjia}}</td>
                    <td>{{$v->shichangjia}}</td>
                    <td>{{$v->chengben}}</td>
					<td>{{$v->shuliang}}</td>
					<td >{!! $v->spjianjie !!}</td>
					<td class="td-manage"> <a style="text-decoration:none" class="ml-5" href="{{url('admin/product/'.$v->id.'/edit')}}" title="编辑"><i style="font-size: 20px" class="Hui-iconfont">&#xe6df;</i></a>
