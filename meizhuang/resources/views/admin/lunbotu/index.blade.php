...
<div >
	...
	<a style="margin-left: 50px;margin-top: 30px" href="javascript:;" onclick="member_add('添加图片','{{url('admin/lunbotu/create')}}','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加图片</a>
	<div class="mt-20">
			<table class="table table-border table-bordered table-bg table-hover table-sort">
				...
				<tr class="text-c va-m">
					<td>{{$v->id}}</td>
                    <td><img width="100" height="50" src="/{{$v->image_url}}"></td>
					<td >{!! $v->content !!}</td>
					<td class="td-manage">
...
<script type="text/javascript">
    function delCate(id){
        layer.confirm('您确定删除吗？', {
            ...
            $.post("{{url('admin/lunbotu/')}}/"+id ,{'_method':'delete','_token':"{{csrf_token()}}"},function (data) {
                if ((data.status==0)) {
